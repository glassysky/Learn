...
function compile(tree, options) {
    ...
    return Promise.all([
        compileDependencyTree(tree, extend(options, { browser: false, readFiles: readFiles })),
        compileDependencyTree(tree, extend(options, { browser: true, readFiles: readFiles }))
    ])
        .then(function (_a) {
...
function resolveFromOverride(src, to) {
    if (typeof to === 'string') {
        if (path_3.isModuleName(to)) {
            var _a = getModuleNameParts(to), moduleName = _a[0], modulePath = _a[1];
...
function resolveFromWithModuleNamePath(src, to) {
    return path_3.isModuleName(to) ? to : path_3.resolveFrom(src, to);
}
    ...
    if (options.browser && browser) {
        if (typeof browser === 'string') {
            ...
            for (var _i = 0, _a = Object.keys(browser); _i < _a.length; _i++) {
                var key = _a[_i];
                var from = resolveFromOverride(tree.src, key);
                var to = resolveFromOverride(tree.src, browser[key]);
                overrides[from] = to;
            }
        }
    }
...
function stringifyDependencyPath(path, options) {
    ...
    var tree = options.tree, ambient = options.ambient, cwd = options.cwd, browser = options.browser, name = options.name, readFiles = options.readFiles, meta = options.meta, entry = options.entry, emitter = options.emitter;
    ...
    function loadByModuleName(path) {
        ...
        var compileOptions = { cwd: cwd, browser: browser, readFiles: readFiles, emitter: emitter, name: moduleName, ambient: false, meta: meta };
...
function importPath(path, name, options) {
    ...
    if (path_3.isModuleName(resolved)) {
        var _a = getModuleNameParts(resolved), moduleName = _a[0], modulePath = _a[1];
